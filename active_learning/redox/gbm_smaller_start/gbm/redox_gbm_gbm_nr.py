...
def get_test_rmse_R2_GBM(x_train_now, y_train_now, x_val_now, y_val_now, x_test, y_test):
...
def get_AL_one_step_result(similarity, x_train, y_train, x_val, y_val, x_test, y_test,
    ...
    for j in range(n_runs):
        ...
        test_rmse_0, test_R2_0, = get_test_rmse_R2_GBM(x_train_now, y_train_now, x_val, y_val, x_test, y_test)

        uncertainty, uncert_prob_dist = get_uncertainty_GBM(x_train_now, y_train_now, x_val, y_val, x_train_remain)
        ...
        test_rmse_one_step, test_R2_one_step = get_test_rmse_R2_GBM(x_train_now, y_train_now, x_val, y_val, x_test, y_test)
...
density_final_rmse_df.to_csv('redox_gbm_gbm_nr_5runs_RMSE_df.csv', header=False, index=False)
density_final_R2_df = pd.DataFrame(density_final_R2_ls)
density_final_R2_df.to_csv('redox_gbm_gbm_nr_5runs_R2_df.csv', header=False, index=False)
