...
'''
def get_uncertainty_MVE(x_train_now, y_train_now, x_val_now, y_val_now, x_train_remain):
    ...
    y_train_remain_pred = model(x_train_remain)
    MVE_unc = np.array(y_train_remain_pred[:, 1])
    uncert_prob_dist = MVE_unc / sum(MVE_unc)

    return MVE_unc, uncert_prob_dist
'''

def get_AL_one_step_result(train_in_bin_idx, train_out_bin_idx, val_in_bin_idx, val_out_bin_idx,
                           test_in_bin_idx,
    ...
    for j in range(n_runs):
        ...
        test_rmse_0, test_rmse_bin_0, test_R2_0, test_R2_bin_0, test_rmse_out_0, test_R2_out_0 = get_test_rmse_R2(x_train_now, y_train_now,
                                                                               x_val_now, y_val_now,
                                                                               x_test, y_test,
                                                                               test_in_bin_idx)

        uncertainty, uncert_prob_dist = get_uncertainty_MCD(x_train_now, y_train_now, x_val_now, y_val_now,
                                                            x_train_remain)
        '''
        ...
        '''
        test_rmse_one_step, test_rmse_bin_one_step, test_R2_one_step, test_R2_bin_one_step, test_rmse_out_one_step, test_R2_out_one_step = get_test_rmse_R2(x_train_now,
                                                                                                           y_train_now,
                                                                                                           x_val_now,
                                                                                                           y_val_now,
                                                                                                           x_test,
        ...
        for j in range(len(nums_mols_to_add)):
            ...
            for k in range(num_PCs):
                ...
                for l in range(num_bins):
                    test_rmse_df, test_rmse_bin_df, test_R2_df, test_R2_bin_df, bin_in_train_count_df, test_rmse_out_df, test_R2_out_df = get_AL_one_step_result(
                        train_in_bin_idx_ls[l],
                        train_out_bin_idx_ls[l],
                        val_in_bin_idx_ls[l],
                        val_out_bin_idx_ls[l],
                        test_in_bin_idx_ls[l],
                        x_train, y_train, x_val, y_val, x_test, y_test,
                        n_runs, starting_percs[i], nums_mols_to_add[j], use_random)
...
density_final_rmse_ls, density_final_rmse_bin_ls, density_final_R2_ls, density_final_R2_bin_ls, density_final_in_bin_count_ls, final_rmse_out_ls, final_R2_out_ls = \
    get_AL_RMSE_matrix(x_train, y_train, x_val, y_val, x_test, y_test,
                              pca_space_train, pca_space_val, pca_space_test,
                              starting_percs, nums_mols_to_add, 3, 5, 20, False)

density_final_rmse_df = pd.DataFrame(density_final_rmse_ls)
density_final_rmse_df.to_csv('sol_mdm_mcd_20runs_RMSE_df.csv', header=False, index=False)
density_final_rmse_bin_df = pd.DataFrame(density_final_rmse_bin_ls)
density_final_rmse_bin_df.to_csv('sol_mdm_mcd_20runs_bin_RMSE_df.csv', header=False, index=False)
density_final_R2_df = pd.DataFrame(density_final_R2_ls)
density_final_R2_df.to_csv('sol_mdm_mcd_20runs_R2_df.csv', header=False, index=False)
density_final_R2_bin_df = pd.DataFrame(density_final_R2_bin_ls)
density_final_R2_bin_df.to_csv('sol_mdm_mcd_20runs_bin_R2_df.csv', header=False, index=False)
density_final_in_bin_count_df = pd.DataFrame(density_final_in_bin_count_ls)
density_final_in_bin_count_df.to_csv('sol_mdm_mcd_20runs_in_bin_count_df.csv', header=False, index=False)

final_rmse_out_df = pd.DataFrame(final_rmse_out_ls)
final_rmse_out_df.to_csv('sol_mdm_mcd_20runs_out_RMSE_df.csv', header=False, index=False)
final_R2_out_df = pd.DataFrame(final_R2_out_ls)
final_R2_out_df.to_csv('sol_mdm_mcd_20runs_out_R2_df.csv', header=False, index=False)

end = time.time()

print(end - start)

# random_final_rmse_ls, random_final_rmse_bin_ls, random_final_R2_ls, random_final_R2_bin_ls, random_final_in_bin_count_ls = \
...
# random_final_rmse_df.to_csv('sol_random_embed_no_seed_3PCs_10runs_RMSE_df.csv', header=False, index=False)
# random_final_rmse_bin_df = pd.DataFrame(random_final_rmse_bin_ls)
# random_final_rmse_bin_df.to_csv('sol_random_embed_no_seed_3PCs_10runs_bin_RMSE_df.csv', header=False, index=False)
# random_final_R2_df = pd.DataFrame(random_final_R2_ls)
# random_final_R2_df.to_csv('sol_random_embed_no_seed_3PCs_10runs_R2_df.csv', header=False, index=False)
# random_final_R2_bin_df = pd.DataFrame(random_final_R2_bin_ls)
# random_final_R2_bin_df.to_csv('sol_random_embed_no_seed_3PCs_10runs_bin_R2_df.csv', header=False, index=False)
# random_final_in_bin_count_df = pd.DataFrame(random_final_in_bin_count_ls)
# random_final_in_bin_count_df.to_csv('sol_random_embed_no_seed_3PCs_10runs_in_bin_count_df.csv', header=False, index=False)
