...
def get_AL_one_step_result(similarity, train_in_bin_idx, train_out_bin_idx, val_in_bin_idx, val_out_bin_idx,
    ...
    for j in range(n_runs):
        ...
        train_in_bin_remain = [i for i, x in enumerate(train_ind_remain) if x in train_in_bin_idx]
        add_ind = random.sample(train_in_bin_remain, num_mols_to_add)
...
density_final_rmse_ls, density_final_rmse_bin_ls, density_final_R2_ls, density_final_R2_bin_ls, density_final_in_bin_count_ls, final_rmse_out_ls, final_R2_out_ls = \
    get_AL_RMSE_matrix(x_train, y_train, x_val, y_val, x_test, y_test, similarity_train_embeddings,
                              pca_space_train, pca_space_val, pca_space_test,
                              starting_percs, nums_mols_to_add, 3, 5, 20, 3, False)

density_final_rmse_df = pd.DataFrame(density_final_rmse_ls)
density_final_rmse_df.to_csv('sol_sampleInBin_no_seed_3PCs_20runs_RMSE_df.csv', header=False, index=False)
density_final_rmse_bin_df = pd.DataFrame(density_final_rmse_bin_ls)
density_final_rmse_bin_df.to_csv('sol_sampleInBin_no_seed_3PCs_20runs_bin_RMSE_df.csv', header=False, index=False)
density_final_R2_df = pd.DataFrame(density_final_R2_ls)
density_final_R2_df.to_csv('sol_sampleInBin_no_seed_3PCs_20runs_R2_df.csv', header=False, index=False)
density_final_R2_bin_df = pd.DataFrame(density_final_R2_bin_ls)
density_final_R2_bin_df.to_csv('sol_sampleInBin_no_seed_3PCs_20runs_bin_R2_df.csv', header=False, index=False)
density_final_in_bin_count_df = pd.DataFrame(density_final_in_bin_count_ls)
density_final_in_bin_count_df.to_csv('sol_sampleInBin_no_seed_3PCs_20runs_in_bin_count_df.csv', header=False, index=False)

final_rmse_out_df = pd.DataFrame(final_rmse_out_ls)
final_rmse_out_df.to_csv('sol_sampleInBin_no_seed_3PCs_20runs_out_RMSE_df.csv', header=False, index=False)
final_R2_out_df = pd.DataFrame(final_R2_out_ls)
final_R2_out_df.to_csv('sol_sampleInBin_no_seed_3PCs_20runs_out_R2_df.csv', header=False, index=False)
...
random_final_rmse_out_df.to_csv('sol_random_embed_no_seed_3PCs_10runs_out_RMSE_df.csv', header=False, index=False)
random_final_R2_out_df = pd.DataFrame(random_final_R2_out_ls)
random_final_R2_out_df.to_csv('sol_random_embed_no_seed_3PCs_10runs_out_R2_df.csv', header=False, index=False)
'''
