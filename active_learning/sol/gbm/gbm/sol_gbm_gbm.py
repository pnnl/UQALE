...
def get_uncertainty_GBM(x_train_now, y_train_now, x_val_now, y_val_now, x_train_remain):
    lower_model = GradientBoostingRegressor(loss="quantile", alpha=LOWER_ALPHA)
    mid_model = GradientBoostingRegressor(loss="ls")
    upper_model = GradientBoostingRegressor(loss="quantile", alpha=UPPER_ALPHA)
    lower_model.fit(x_train_now, y_train_now)
    ...
    predictions['lower'] = lower_model.predict(x_train_remain)
    predictions['mid'] = mid_model.predict(x_train_remain)
    predictions['upper'] = upper_model.predict(x_train_remain)

    GB_unc = np.array(np.sqrt(abs(predictions['upper'] - predictions['lower']) / 2))

    uncert_prob_dist = GB_unc / sum(GB_unc)

    return GB_unc, uncert_prob_dist
...
def get_AL_one_step_result(similarity, train_in_bin_idx, train_out_bin_idx, val_in_bin_idx, val_out_bin_idx,
    ...
    for j in range(n_runs):
        ...
        uncertainty, uncert_prob_dist = get_uncertainty_GBM(x_train_now, y_train_now, x_val_now, y_val_now, x_train_remain)
        '''
...
density_final_rmse_ls, density_final_rmse_bin_ls, density_final_R2_ls, density_final_R2_bin_ls, density_final_in_bin_count_ls, final_rmse_out_ls, final_R2_out_ls = \
    get_AL_RMSE_matrix(x_train, y_train, x_val, y_val, x_test, y_test, similarity_train_embeddings,
                              pca_space_train, pca_space_val, pca_space_test,
                              starting_percs, nums_mols_to_add, 3, 5, 5, 3, False)

density_final_rmse_df = pd.DataFrame(density_final_rmse_ls)
density_final_rmse_df.to_csv('sol_gbm_gbm_5runs_RMSE_df.csv', header=False, index=False)
density_final_rmse_bin_df = pd.DataFrame(density_final_rmse_bin_ls)
density_final_rmse_bin_df.to_csv('sol_gbm_gbm_5runs_bin_RMSE_df.csv', header=False, index=False)
density_final_R2_df = pd.DataFrame(density_final_R2_ls)
density_final_R2_df.to_csv('sol_gbm_gbm_5runs_R2_df.csv', header=False, index=False)
density_final_R2_bin_df = pd.DataFrame(density_final_R2_bin_ls)
density_final_R2_bin_df.to_csv('sol_gbm_gbm_5runs_bin_R2_df.csv', header=False, index=False)
density_final_in_bin_count_df = pd.DataFrame(density_final_in_bin_count_ls)
density_final_in_bin_count_df.to_csv('sol_gbm_gbm_5runs_in_bin_count_df.csv', header=False, index=False)

final_rmse_out_df = pd.DataFrame(final_rmse_out_ls)
final_rmse_out_df.to_csv('sol_gbm_gbm_5runs_out_RMSE_df.csv', header=False, index=False)
final_R2_out_df = pd.DataFrame(final_R2_out_ls)
final_R2_out_df.to_csv('sol_gbm_gbm_5runs_out_R2_df.csv', header=False, index=False)
...
random_final_rmse_df.to_csv('sol_random_GBM_no_seed_3PCs_10runs_RMSE_df_GBMpred.csv', header=False, index=False)
random_final_rmse_bin_df = pd.DataFrame(random_final_rmse_bin_ls)
random_final_rmse_bin_df.to_csv('sol_random_GBM_no_seed_3PCs_10runs_bin_RMSE_df_GBMpred.csv', header=False, index=False)
random_final_R2_df = pd.DataFrame(random_final_R2_ls)
random_final_R2_df.to_csv('sol_random_GBM_no_seed_3PCs_10runs_R2_df_GBMpred.csv', header=False, index=False)
random_final_R2_bin_df = pd.DataFrame(random_final_R2_bin_ls)
random_final_R2_bin_df.to_csv('sol_random_GBM_no_seed_3PCs_10runs_bin_R2_df_GBMpred.csv', header=False, index=False)
random_final_in_bin_count_df = pd.DataFrame(random_final_in_bin_count_ls)
random_final_in_bin_count_df.to_csv('sol_random_GBM_no_seed_3PCs_10runs_in_bin_count_df_GBMpred.csv', header=False, index=False)
'''
