...
#similarity_all = # fingerprint similarity data path
...
sol_cos_mat = # embedding similarity data path
sol_cos_mat = sol_cos_mat['arr_0']
similarity_test_embedding_np = sol_cos_mat[15862:, 0:14576]

similarity_test_np = similarity_test_embedding_np.copy()
...
err_perc_pc = np.array([ 0.14824486,  0.01951019,  0.01442217, -0.0285143 ,  0.09481517,
        0.23763902,  0.01310516,  0.10739236,  0.09996919,  0.09669765,
        0.06372875,  0.06130176,  0.06453355, -0.00409051,  0.12850164])



def get_density_test_unc(similarity_test_np, train_ind_remain, num_NNs):
    similarity_temp = similarity_test_np[:, train_ind_remain]
...
def get_unc_bin_revomal_results_density(similarity_test_np, num_NNs,
    ...
    for j in range(n_exp):
        ...
        for i in range(len(train_in_bin_idx)):
            ...
            test_total_uncertainty = unc_func_GNN(similarity_test_np, train_ind_remain, num_NNs)
...
def get_unc_result(similarity_test_np, num_NNs,
    ...
    for k in range(num_pc):
        ...
        test_unc_in_bin_df, test_unc_remain_df, test_unc_in_bin_nr_df, test_unc_remain_nr_df = \
            get_unc_bin_revomal_results_density(similarity_test_np, num_NNs,
...
unc_perc_density, test_unc_in_bin_df_ls, test_unc_remain_df_ls, test_unc_in_bin_nr_df_ls, test_unc_remain_nr_df_ls = get_unc_result(similarity_test_np, 3,
                                     pca_space_train, pca_space_val, pca_space_test, 3, 5, 1, get_density_test_unc)

np.savez('sol_embed.npz', unc_perc_density, test_unc_in_bin_df_ls,
         test_unc_remain_df_ls,
         test_unc_in_bin_nr_df_ls,
         test_unc_remain_nr_df_ls)

#print(pearsonr(err_perc_pc, unc_perc_density))
#print(spearmanr(err_perc_pc, unc_perc_density))
