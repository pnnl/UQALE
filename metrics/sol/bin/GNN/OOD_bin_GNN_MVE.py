...
def test_func_MVE_train(loader, model, device):
    model.eval()
    with torch.no_grad():
        target, predicted = [], []
        for data in loader:
...
def test_func_MVE(loader, model, device):
    model.eval()
    with torch.no_grad():
...
class DenseNormal(nn.Module):
    def __init__(self, units):
        ...
        self.dense = nn.Linear(256, 2 * self.units)
...
def get_test_uncertainty_GNN_MVE(train_loader_now, val_loader_now, test_loader_no_shuffle):
    ...
    for epoch in range(0, n_epochs):
        ...
        val_rmse = test_func_MVE_train(val_loader_now, model, device)
    ...
    target, predicted, predicted_unc = test_func_MVE(test_loader_no_shuffle, model, device)

    return predicted_unc
...
unc_perc_GNN_MVE, test_unc_in_bin_df_ls, test_unc_remain_df_ls, test_unc_in_bin_nr_df_ls, test_unc_remain_nr_df_ls = get_unc_result(train_X, val_X, train_loader, val_loader, test_loader_no_shuffle,
                                  pca_space_train, pca_space_val, pca_space_test, 3, 5, 1, get_test_uncertainty_GNN_MVE)
np.savez('sol_gnn_mve.npz', unc_perc_GNN_MVE, test_unc_in_bin_df_ls,
         test_unc_remain_df_ls,
         test_unc_in_bin_nr_df_ls,
         test_unc_remain_nr_df_ls)
