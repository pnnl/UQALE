...
def predict_dist_ensemble(loader, model_ensemble, device):
    test_preds = [test_func_predicted(loader, model_ensemble[i], device) for i in range(10)]
...
def get_unc_result(train_X, val_X, train_loader, val_loader, test_loader_no_shuffle,
    ...
    return np.array(unc_perc).reshape(1, num_bins * num_pc)[0], test_unc_in_bin_df_ls, test_unc_remain_df_ls, test_unc_in_bin_nr_df_ls, test_unc_remain_nr_df_ls
...
np.savez('sol_gnn_ens.npz', unc_perc_ens, test_unc_in_bin_df_ls,
         test_unc_remain_df_ls,
         test_unc_in_bin_nr_df_ls,
         test_unc_remain_nr_df_ls)
