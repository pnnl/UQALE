...
def test_func_MVE_unc(loader, model):
    model.eval()
    with torch.no_grad():
...
target, predicted, predicted_unc = test_func_MVE_unc(test_loader, model)
...
for i in range(10):
    l = MVE_unc_ordered[(i * 128):((i + 1) * 128)]
    RMV_i = math.sqrt(sum(map(lambda x: x * x, l)) / len(l))
    RMV.append(RMV_i)

    p = y_pred[MVE_unc_ordered_idx[(i * 128):((i + 1) * 128)]]
    t = y_test[MVE_unc_ordered_idx[(i * 128):((i + 1) * 128)]]
...
print(ENCE(RMV, RMSE))
print(spearmanr(MVE_unc, abs(np.array(y_pred) - y_test)))
print(pearsonr(MVE_unc, abs(np.array(y_pred) - y_test)))
