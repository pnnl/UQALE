...
def test_func_edl(loader, model):
    model.eval()
    with torch.no_grad():
...
class DenseNormalGamma(nn.Module):
    def __init__(self, size_in):
        ...
        self.dense = nn.Linear(256, 4 * self.units) # nn.Linear(256, 1) # nn.Linear(256, 4 * self.units)
...
if retrain:
    ...
    #     train_func(epoch)
        ...
        for data in train_loader:
            ...
            loss = EvidentialRegression(gamma, v, alpha, beta, data.y, coeff=0.0025) # criterion(gamma, data.y) #
            loss.backward()
            optimizer.step()
...
target, predicted, predicted_v, predicted_alpha, predicted_beta = test_func_edl(test_loader, model)
...
for i in range(10):
    l = total_uncertainty_ordered[(i * 129):((i + 1) * 129)]
    RMV_i = math.sqrt(sum(map(lambda x: x * x, l)) / len(l))
    RMV.append(RMV_i)

    p = y_pred[total_uncertainty_ordered_idx[(i * 129):((i + 1) * 129)]]
    t = y_test[total_uncertainty_ordered_idx[(i * 129):((i + 1) * 129)]]
...
for i in range(10):
    l = data_uncertainty_ordered[(i * 129):((i + 1) * 129)]
    RMV_i = math.sqrt(sum(map(lambda x: x * x, l)) / len(l))
    RMV_data.append(RMV_i)

    p = y_pred[data_uncertainty_ordered_idx[(i * 129):((i + 1) * 129)]]
    t = y_test[data_uncertainty_ordered_idx[(i * 129):((i + 1) * 129)]]
    RMSE_i = math.sqrt(sum((p - t) ** 2) / len(p))
    RMSE_data.append(RMSE_i)
...
for i in range(10):
    l = model_uncertainty_ordered[(i * 129):((i + 1) * 129)]
    RMV_i = math.sqrt(sum(map(lambda x: x * x, l)) / len(l))
    RMV_model.append(RMV_i)

    p = y_pred[model_uncertainty_ordered_idx[(i * 129):((i + 1) * 129)]]
    t = y_test[model_uncertainty_ordered_idx[(i * 129):((i + 1) * 129)]]
    RMSE_i = math.sqrt(sum((p - t) ** 2) / len(p))
    RMSE_model.append(RMSE_i)
...
print(ENCE(RMV, RMSE))
y_test = test['log_sol']
print(spearmanr(total_uncertainty, abs(predicted - y_test)))
print(pearsonr(total_uncertainty, abs(predicted - y_test)))
