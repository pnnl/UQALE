...
def test_func_mcdo(loader, model):
    model.train()
    with torch.no_grad():
        target, predicted = [], []
        for data in loader:
    ...
    return target, predicted # mean_squared_error(y_true=target, y_pred=predicted)
...
predicted_ls = []
for _ in range(100):
    target, predicted = test_func_mcdo(test_loader, model)
    predicted_ls.append(predicted)

predicted_df = pd.DataFrame(predicted_ls)
mcd_std = predicted_df.std(axis=0)
abs_error = abs(predicted_df.mean(axis=0) - target)

print(spearmanr(mcd_std, abs_error))
print(pearsonr(mcd_std, abs_error))

y_pred = predicted_df.mean(axis=0)
y_test = test['log_sol']

total_uncertainty = mcd_std
