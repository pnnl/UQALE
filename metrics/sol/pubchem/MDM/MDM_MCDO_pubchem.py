...
mc = ModelCheckpoint('sol_mdm_best_model_pubchem.h5', monitor='val_loss', mode='min', save_best_only=True, verbose=0)

result = model.fit(x_train, y_train, batch_size = 64, epochs = 1000,
          verbose = 0, validation_data = (x_val,y_val), callbacks = [es,mc])
...
y_pred_dist_pubchem = predict_dist(pubchem_max_train, model, 100)
y_pred_pubchem = predict_point(pubchem_max_train, model, 100)
...
print(spearmanr(y_pred_dist_std_pubchem, pubchem_orig_sim))
print(pearsonr(y_pred_dist_std_pubchem, pubchem_orig_sim))
