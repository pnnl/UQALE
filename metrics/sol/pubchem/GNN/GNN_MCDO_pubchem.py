...
predicted_ls = []
for _ in range(50):
    target, predicted = test_func_pubchem(pubchem_loader, model)
    predicted_ls.append(predicted)

predicted_df = pd.DataFrame(predicted_ls)
mcd_std_pubchem = predicted_df.std(axis=0)
...
print(spearmanr(mcd_std_pubchem, pubchem_orig_sim))
print(pearsonr(mcd_std_pubchem, pubchem_orig_sim))
