...
def test_func_mcd(loader, model):
    model.train()
...
predicted_pubchem_ls = []
for _ in range(30):
    predicted = test_func_mcd(pubchem_loader, model)
    predicted_pubchem_ls.append(predicted)
predicted_pubchem_df = pd.DataFrame(predicted_pubchem_ls)
mcd_std_pubchem = predicted_pubchem_df.std(axis=0)
spearmanr(mcd_std_pubchem, redox_pubchem['max_sim'])
pearsonr(mcd_std_pubchem, redox_pubchem['max_sim'])
