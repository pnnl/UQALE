...
def predict_dist_ensemble(X, model_ensemble):
    preds = [model_ensemble[i].predict(X) for i in range(15)]
...
y_pred_dist_ensemble_pubchem = predict_dist_ensemble(redox_pubchem_mdm_sc, model_ensemble)
y_pred_ensemble_pubchem = predict_point_ensemble(redox_pubchem_mdm_sc, model_ensemble)
