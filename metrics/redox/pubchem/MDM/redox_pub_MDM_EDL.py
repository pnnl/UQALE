...
def get_EDL_unc(model, x_test):
    ...
    return np.array(y_test_pred), np.array(data_uncertainty), np.array(model_uncertainty), np.array(total_uncertainty)

y_test_pred_pubchem, data_uncertainty_pubchem, model_uncertainty_pubchem, total_uncertainty_pubchem = get_EDL_unc(model, redox_pubchem_mdm_sc)
