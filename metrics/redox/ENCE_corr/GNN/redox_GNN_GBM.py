...
test_p_res = test_p.reshape(7388, 96)

predictions = pd.DataFrame(test_t)
...
part_len = round(len(GB_unc) / 10)

for i in range(10):
    l = GB_unc_ordered[(i * part_len):((i + 1) * part_len)]
    RMV_i = math.sqrt(sum(map(lambda x: x * x, l)) / len(l))
    RMV.append(RMV_i)

    p = predictions['mid'][GB_unc_ordered_idx[(i * part_len):((i + 1) * part_len)]]
    t = test_t[GB_unc_ordered_idx[(i * part_len):((i + 1) * part_len)]]
...
print(spearmanr(GB_unc, abs(predictions['mid'] - test_t)))
print(pearsonr(GB_unc, abs(predictions['mid'] - test_t)))
