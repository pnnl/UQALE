...
predicted_ls = []
for _ in range(30):
    target, predicted = test_func(test_loader, model)
...
abs_error = abs(predicted_mean - target)

spearmanr(mcd_std, abs_error)
pearsonr(mcd_std, abs_error)

total_uncertainty = mcd_std.copy()
...
for i in range(10):
    ...
    p = np.array(predicted_mean)[total_uncertainty_ordered_idx[(i * part_len):((i + 1) * part_len)]]
