...
target, predicted = test_func_plotting(test_loader, model_ensemble[0], device)
...
part_len = round(len(y_pred_dist_std_ensemble)/10)

#i=0
for i in range(10):
    l = y_pred_dist_std_ordered_ensemble[(i*part_len):((i+1)*part_len)]
    RMV_i = math.sqrt(sum(map(lambda x:x*x, l))/len(l))
    RMV_ensemble.append(RMV_i)
    
    p = y_pred_ensemble[y_pred_dist_std_ordered_idx_ensemble[(i*part_len):((i+1)*part_len)]]
    t = target[y_pred_dist_std_ordered_idx_ensemble[(i*part_len):((i+1)*part_len)]]
...
print(stats.spearmanr(y_pred_dist_std_ensemble, abs(y_pred_ensemble - target)))
print(stats.pearsonr(y_pred_dist_std_ensemble, abs(y_pred_ensemble - target)))
