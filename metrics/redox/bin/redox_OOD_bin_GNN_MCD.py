...
def test_func_mcd(loader, model, device):
    model.train()
...
def get_val_uncertainty_GNN_MCD(train_loader_now, val_loader_now, test_loader_no_shuffle):
    ...
    predicted_ls = []
    for _ in range(100):
        test_t, test_p = test_func_mcd(test_loader_no_shuffle, model, device)
        predicted_ls.append(test_p)
    predicted_df = pd.DataFrame(predicted_ls)
    mcd_std = predicted_df.std(axis=0)
...
unc_perc_GNN_MCD, test_unc_in_bin_df_ls, test_unc_remain_df_ls, test_unc_in_bin_nr_df_ls, test_unc_remain_nr_df_ls = \
    get_unc_result(train_X, val_X, train_loader, val_loader, test_loader_no_shuffle,
                   pca_space_train, pca_space_val, pca_space_test, 3, 5, 1, get_val_uncertainty_GNN_MCD)


np.savez('redox_gnn_mcd.npz', unc_perc_GNN_MCD, test_unc_in_bin_df_ls,
         test_unc_remain_df_ls,
         test_unc_in_bin_nr_df_ls,
         test_unc_remain_nr_df_ls)

print(pearsonr(err_perc_pc, unc_perc_GNN_MCD))
print(spearmanr(err_perc_pc, unc_perc_GNN_MCD))
