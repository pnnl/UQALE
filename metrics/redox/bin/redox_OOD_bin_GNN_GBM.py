...
def get_test_uncertainty_GNN_GBM(train_X_now, train_loader_now, train_loader_now_no_shuffle,
                                val_loader_now, test_loader_no_shuffle):
    ...
    test_p_res = test_p.reshape(len(test_X), 96)
    train_t, train_p = test_func_plotting(train_loader_now_no_shuffle, model, device)
    train_p_res = train_p.reshape(len(train_X_now), 96)
    ...
    upper_model = GradientBoostingRegressor(loss="quantile",
                                            alpha=UPPER_ALPHA)
    lower_model.fit(train_p_res, train_t)
    mid_model.fit(train_p_res, train_t)
    upper_model.fit(train_p_res, train_t)

    predictions = pd.DataFrame()
    predictions['lower'] = lower_model.predict(test_p_res)
    predictions['mid'] = mid_model.predict(test_p_res)
    predictions['upper'] = upper_model.predict(test_p_res)
...
def get_unc_bin_revomal_results_GNN(train_X, val_X, train_loader, train_loader_no_shuffle, val_loader, test_loader_no_shuffle,
    ...
    for j in range(n_exp):
        ...
        for i in range(len(train_in_bin_idx)):
            ...
            train_loader_now_no_shuffle = DataLoader(train_X_now, batch_size=bs, shuffle=False, drop_last=False)
            ...
            # val_total_uncertainty = unc_func_GNN(train_loader_now, val_loader_now, val_loader_no_shuffle)
            test_total_uncertainty = unc_func_GNN(train_X_now, train_loader_now, train_loader_now_no_shuffle,
                                                 val_loader_now, test_loader_no_shuffle)
        ...
        test_total_uncertainty = unc_func_GNN(train_X, train_loader, train_loader_no_shuffle,
                                              val_loader, test_loader_no_shuffle)
...
def get_unc_result(train_X, val_X, train_loader, train_loader_no_shuffle, val_loader, test_loader_no_shuffle,
    ...
    for k in range(num_pc):
        ...
        test_unc_in_bin_df, test_unc_remain_df, test_unc_in_bin_nr_df, test_unc_remain_nr_df = \
            get_unc_bin_revomal_results_GNN(train_X, val_X, train_loader, train_loader_no_shuffle, val_loader,
                                            test_loader_no_shuffle, val_in_bin_idx_ls, test_in_bin_idx_ls, train_in_bin_idx_ls,
                                            n_exp, unc_func_GNN)
...
unc_perc_GNN_GBM, test_unc_in_bin_df_ls, test_unc_remain_df_ls, test_unc_in_bin_nr_df_ls, test_unc_remain_nr_df_ls = \
    get_unc_result(train_X, val_X, train_loader, train_loader_no_shuffle,
                   val_loader, test_loader_no_shuffle,
                   pca_space_train, pca_space_val, pca_space_test, 3, 5, 1, get_test_uncertainty_GNN_GBM)

np.savez('redox_gnn_gbm.npz', unc_perc_GNN_GBM, test_unc_in_bin_df_ls,
         test_unc_remain_df_ls,
         test_unc_in_bin_nr_df_ls,
         test_unc_remain_nr_df_ls)

print(pearsonr(err_perc_pc, unc_perc_GNN_GBM))
print(spearmanr(err_perc_pc, unc_perc_GNN_GBM))
